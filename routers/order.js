...
router.get("/orders/pending_orders",authenthicate("user"),async (req,res)=>{
  try{
    let user_orders= await Orders.find({user:req.user.id,process:"in progress"});
...
router.get("/orders/cancelled_orders",authenthicate("user"),async (req,res)=>{
  try{
    let user_orders= await Orders.find({user:req.user.id,process:"cancelled"});
    //let order = await Orders.findById("5d69968ce05c0a62e9be1c4d");
    //first get the list of product in the cart by looping through it and get the name,total quantity,each price of product
    let totalorders = [];
...
router.get("/orders/:id",authenthicate("user"),async(req,res)=>{
  try{
